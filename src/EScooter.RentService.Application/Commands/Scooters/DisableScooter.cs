...
/// <summary>
/// An application command to disable a scooter.
/// </summary>
public class DisableScooter
{
    /// <summary>
    /// The command data for the <see cref="EScooter.RentService.Application.Commands.Scooters.DisableScooter"/> command.
    /// </summary>
    /// <param name="ScooterId">The Id of the scooter to disable.</param>
    public record Command(Guid ScooterId) : CommandBase<Nothing>;

    /// <summary>
    /// The handler for the <see cref="DisableScooter"/> command.
    /// </summary>
    public class Handler : UnitOfWorkHandler<Command, Nothing>
    {
        ...
        protected override async Task<Response<Nothing>> HandleRequest(Command request)
        {
            return await _scooterRepository.GetById(request.ScooterId)
                .ThenIfSuccess(scooter => scooter.Disable())
